...
stripe_endpoint_secret = os.getenv(
    'STRIPE_ENDPOINT_SECRET', GCP_Secret_Manager_Service().get_secret('STRIPE_ENDPOINT_SECRET'))
...
class Client_Model(db.Model):
    ...
    dietitian_id = db.Column(db.String(80), db.ForeignKey(
        'dietitian.id'), nullable=False)
    meal_plan_id = db.Column(UUID(as_uuid=True), db.ForeignKey(
        'meal_plan.id'), nullable=False)
    ...
    meal_plan = relationship(
        'Meal_Plan_Model',  lazy=True)
    meal_subscription = relationship(
        'Meal_Subscription_Model', lazy=True)
...
class State_Sales_Tax_Model(db.Model):
    ...
    def __init__(self, state: str, sales_tax_percentage: float, stripe_tax_id: str) -> None:
        self.state = state
...
class Meal_Plan_Model(db.Model):
    ...
    def __init__(self, meal_plan_dict: dict) -> None:
        ...
        self.per_snack_caloric_lower_bound = meal_plan_dict['per_snack_caloric_lower_bound']
        self.per_snack_caloric_upper_bound = meal_plan_dict['per_snack_caloric_upper_bound']
...
class Meal_Model(db.Model):
    ...
    dietary_restrictions = relationship(
        'Meal_Dietary_Restriction_Model', lazy=True)
...
class Meal_Plan_Meal_Model(db.Model):
    ...
    id = db.Column(UUID(as_uuid=True), primary_key=True,
                   unique=True, nullable=False)
    meal_id = db.Column(UUID(as_uuid=True),
                        db.ForeignKey('meal.id'), primary_key=True, nullable=False)
    meal_plan_id = db.Column(UUID(as_uuid=True),
                             db.ForeignKey('meal_plan.id'), primary_key=True, nullable=False)
    active = db.Column(db.Boolean(), default=True, nullable=False)
    recipe = relationship('Recipe_Ingredient_Model', lazy='joined')
    associated_meal = relationship(
        'Meal_Model',  lazy='joined')
    associated_meal_plan = relationship(
        'Meal_Plan_Model',  lazy='joined')
...
class Order_Meal_Model(db.Model):
    ...
    id = db.Column(UUID(as_uuid=True), primary_key=True,
                   unique=True, nullable=False)
    meal_subscription_invoice_id = db.Column(UUID(as_uuid=True), db.ForeignKey('meal_subscription_invoice.id'),
                                             nullable=False)
    scheduled_order_meal_id = db.Column(UUID(as_uuid=True), db.ForeignKey('scheduled_order_meal.id'),
                                        nullable=True)
    # Relationships
    scheduled_order_meal = relationship(
        'Scheduled_Order_Meal_Model', lazy='joined')
...
class Meal_Subscription_Invoice_Model(db.Model):
    ...
    id = db.Column(UUID(as_uuid=True), primary_key=True,
                   unique=True, nullable=False)
    meal_subscription_id = db.Column(UUID(as_uuid=True), db.ForeignKey('meal_subscription.id'),
                                     nullable=False)
    ...
    def __init__(self, meal_subscription_invoice_domain: 'Meal_Subscription_Invoice_Domain') -> None:
        self.id = meal_subscription_invoice_domain.id
        self.meal_subscription_id = meal_subscription_invoice_domain.meal_subscription_id
        self.subtotal = meal_subscription_invoice_domain.subtotal
        self.sales_tax_percentage = meal_subscription_invoice_domain.sales_tax_percentage
        ...
        self.stripe_payment_intent_id = meal_subscription_invoice_domain.stripe_payment_intent_id
...
class Scheduled_Order_Meal_Model(db.Model):
    ...
    meal_subscription_id = db.Column(UUID(as_uuid=True), db.ForeignKey('meal_subscription.id'),
                                     nullable=False)
    meal_id = db.Column(UUID(as_uuid=True), db.ForeignKey('meal.id'),
                        nullable=False)
    delivery_date = db.Column(db.Float(), nullable=False)
    ...
    def __init__(self, scheduled_order_meal_domain: 'Scheduled_Order_Meal_Domain') -> None:
...
class Staged_Schedule_Meal_Model(db.Model):
    ...
    meal_id = db.Column(UUID(as_uuid=True),
                        db.ForeignKey('meal.id'), nullable=False)
    staged_client_id = db.Column(db.String(80), db.ForeignKey('staged_client.id'),
                                 nullable=False)
    dietitian_prepayment_id = db.Column(UUID(as_uuid=True),
                                        db.ForeignKey('dietitian_prepayment.id'), nullable=True)

    associated_meal = relationship(
        'Meal_Model',  lazy='joined')

    def __init__(self, staged_schedule_meal_domain: 'Staged_Schedule_Meal_Domain') -> None:
...
class Meal_Subscription_Model(db.Model):
    ...
    id = db.Column(UUID(as_uuid=True), primary_key=True,
                   unique=True, nullable=False)
    client_id = db.Column(db.String(80), db.ForeignKey(
        'client.id'), nullable=False)
    dietitian_id = db.Column(db.String(80), db.ForeignKey(
        'dietitian.id'), nullable=False)
    stripe_subscription_id = db.Column(db.String(80), unique=True,
                                       nullable=False)
...
class Meal_Dietary_Restriction_Model(db.Model):
    ...
    id = db.Column(UUID(as_uuid=True), primary_key=True,
                   unique=True, nullable=False)
    dietary_restriction_id = db.Column(db.String(80), db.ForeignKey(
        'dietary_restriction.id'), nullable=False)
    meal_id = db.Column(UUID(as_uuid=True), db.ForeignKey(
        'meal.id'), nullable=False)

    def __init__(self, meal_dietary_restriction_domain: 'Meal_Dietary_Restriction_Domain') -> None:
        self.id = meal_dietary_restriction_domain.id
        self.dietary_restriction_id = meal_dietary_restriction_domain.dietary_restriction_id
...
class Dietary_Restriction_Model(db.Model):
    ...
    meal_dietary_restriction: Mapped[list[Meal_Dietary_Restriction_Model]] = relationship(
        'Meal_Dietary_Restriction_Model', lazy=True)
...
class USDA_Ingredient_Model(db.Model):
    ...
    name = db.Column(db.String(80),
                     unique=True, nullable=False)
    fdc_id = db.Column(db.String(80),
                       unique=True, nullable=False)
    fda_identifier = db.Column(db.String(80),
                               unique=True, nullable=False)
    ...
    def __init__(self, usda_ingredient_nutrient_mapper: 'USDA_Nutrient_Mapper_DTO') -> None:
        ...
        self.k_cal_to_gram_ratio = usda_ingredient_nutrient_mapper.calories_to_grams_ratio
...
class Recipe_Ingredient_Model(db.Model):
    ...
    id = db.Column(UUID(as_uuid=True), primary_key=True,
                   unique=True, nullable=False)
    usda_ingredient_id = db.Column(db.String(80), db.ForeignKey(
        'usda_ingredient.id'), primary_key=True, nullable=False)
    meal_plan_meal_id = db.Column(UUID(as_uuid=True), db.ForeignKey(
        'meal_plan_meal.id'), primary_key=True, nullable=False)
    usda_ingredient_portion_id = db.Column(UUID(as_uuid=True), db.ForeignKey(
        'usda_ingredient_portion.id'), nullable=False)
    ...
    usda_ingredient = relationship(
        'USDA_Ingredient_Model', lazy=True)
    ...
    def __init__(self, recipe_ingredient_domain: 'Recipe_Ingredient_Domain') -> None:
        ...
        self.usda_ingredient_portion_id = recipe_ingredient_domain.usda_ingredient_portion_id
...
class USDA_Ingredient_Portion_Model(db.Model):
    ...
    id = db.Column(UUID(as_uuid=True), primary_key=True,
                   unique=True, nullable=False)
    usda_ingredient_id = db.Column(db.String(80), db.ForeignKey(
        'usda_ingredient.id'), nullable=False)
    fda_portion_id = db.Column(
...
class Nutrient_Model(db.Model):
    ...
    name = db.Column(db.String(80), primary_key=True,
                     unique=True, nullable=False)
    usda_id = db.Column(db.String(80),
                        unique=True, nullable=False)
...
class USDA_Ingredient_Nutrient_Model(db.Model):
    ...
    usda_ingredient_id = db.Column(db.String(80), db.ForeignKey(
        'usda_ingredient.id'), nullable=False)
    nutrient_id = db.Column(db.String(80), db.ForeignKey(
...
class Recipe_Ingredient_Nutrient_Model(db.Model):
    ...
    id = db.Column(UUID(as_uuid=True), primary_key=True,
                   unique=True, nullable=False)
    recipe_ingredient_id = db.Column(UUID(as_uuid=True), db.ForeignKey(
    ...
    nutrient: Mapped[Nutrient_Model] = relationship(
        'Nutrient_Model',  lazy='joined')

    def __init__(self, recipe_ingredient_nutrient_domain: 'Recipe_Ingredient_Nutrient_Domain') -> None:
        self.id = recipe_ingredient_nutrient_domain.id
        self.recipe_ingredient_id = recipe_ingredient_nutrient_domain.recipe_ingredient_id
        self.nutrient_id = recipe_ingredient_nutrient_domain.nutrient_id
        self.usda_nutrient_daily_value_amount = recipe_ingredient_nutrient_domain.usda_nutrient_daily_value_amount
...
class USDA_Nutrient_Daily_Value_Model(db.Model):
    ...
    id = db.Column(UUID(as_uuid=True), primary_key=True,
                   unique=True, nullable=False)
    nutrient_id = db.Column(db.String(80), db.ForeignKey(
        'nutrient.id'), primary_key=True, nullable=False)
    meal_plan_id = db.Column(UUID(as_uuid=True), db.ForeignKey(
        'meal_plan.id'), primary_key=True, nullable=False)
    amount = db.Column(db.Float(), nullable=False)
    unit = db.Column(db.String(80), nullable=False)
...
class Prepaid_Order_Discount_Model(db.Model):
    ...
    discount_id = db.Column(UUID(as_uuid=True), db.ForeignKey('discount.id'), primary_key=True,
                            nullable=False)
    dietitian_prepayment_id = db.Column(UUID(as_uuid=True), db.ForeignKey('dietitian_prepayment.id'), primary_key=True,
                                        nullable=False)
    amount = db.Column(db.Float(), nullable=False)
    datetime = db.Column(db.Float(), nullable=False)

    def __init__(self, prepaid_order_discount_domain: 'Prepaid_Order_Discount_Domain') -> None:
        self.discount_id = prepaid_order_discount_domain.discount_id
        self.dietitian_prepayment_id = prepaid_order_discount_domain.dietitian_prepayment_id
...
class Order_Discount_Model(db.Model):
    ...
    discount_id = db.Column(UUID(as_uuid=True), db.ForeignKey('discount.id'), primary_key=True,
                            nullable=False)
    staged_client_id = db.Column(db.String(80), db.ForeignKey(
        'staged_client.id'), primary_key=True, nullable=False)
    amount = db.Column(db.Float(), nullable=False)
    datetime = db.Column(db.Float(), nullable=False)
...
class Dietitian_Prepayment_Model(db.Model):
    ...
    id = db.Column(UUID(as_uuid=True), primary_key=True, unique=True,
                   nullable=False)
    dietitian_id = db.Column(db.String(80), db.ForeignKey(
        'dietitian.id'), primary_key=True, nullable=False)
    staged_client_id = db.Column(db.String(80), db.ForeignKey(
        'staged_client.id'), primary_key=True, nullable=False)
    subtotal = db.Column(db.Float(), nullable=False)
    sales_tax_percentage = db.Column(db.Float(), nullable=False)
    ...
    def __init__(self, dietitian_prepayment_domain: 'Dietitian_Prepayment_Domain') -> None:
...
def create_state_tax_rates() -> None:
    ...
    states = [{'state': 'NJ', 'sales_tax': 0.0625},
    ...
    for state in states:
        ...
        stripe_state_tax_rate = stripe.TaxRate.create(
            display_name=f'{state_name} Sales Tax', inclusive=False, percentage=stripe_state_sales_tax_percentage)

        new_state_sales_tax = State_Sales_Tax_Model(
            state=state_name, sales_tax_percentage=state_sales_tax_percentage, stripe_tax_id=stripe_state_tax_rate.id)
...
def create_new_usda_ingredient_nutrients() -> None:
    ...
    for usda_ingredient_nutrient in usda_ingredient_nutrients:
        if usda_ingredient_nutrient['nutrient_id'] != 'net_carb' and usda_ingredient_nutrient['nutrient_id'] != 'pantothenic_acid' and usda_ingredient_nutrient['nutrient_id'] != 'biotin':
...
def create_new_meal_plan_meals() -> None:
    ...
    for meal_plan_meal in meal_plan_meals:
        new_meal_plan_meal = Meal_Plan_Meal_Model(
            meal_plan_meal_dict=meal_plan_meal)
...
def create_new_recipe_ingredient_nutrients() -> None:
    ...
    for recipe_ingredient_nutrient in recipe_ingredient_nutrients:
        if recipe_ingredient_nutrient['nutrient_id'] != 'net_carb' and recipe_ingredient_nutrient['nutrient_id'] != 'pantothenic_acid' and recipe_ingredient_nutrient['nutrient_id'] != 'biotin':
...
def create_discount() -> None:
    new_discount_dict = {'id': uuid.uuid4(), 'code': 'FNCE 2022',
                         'discount_percentage': 0.5, 'active': True}
