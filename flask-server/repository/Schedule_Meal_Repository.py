...
class Schedule_Meal_Repository(Base_Repository):
    def create_schedule_meals(
        self, schedule_meal_domains: list["Schedule_Meal_Domain"]
    ) -> None:
        for schedule_meal_domain in schedule_meal_domains:
            new_schedule_meal = Schedule_Meal_Model(
                id=schedule_meal_domain.id,
                meal_id=schedule_meal_domain.meal_id,
                meal_subscription_id=schedule_meal_domain.meal_subscription_id,
            )
    ...
    ) -> Optional[list[Schedule_Meal_Model]]:
        if meal_subscription_id:
            schedule_meals: list[Schedule_Meal_Model] = (
                self.db.session.query(Schedule_Meal_Model)
                .filter(
                    Schedule_Meal_Model.meal_subscription_id == meal_subscription_id
                )
                .all()
            )
        else:
    ...
    def delete_schedule_meals(self, meal_subscription_id: UUID) -> None:
        schedule_meals_to_delete: list[Schedule_Meal_Model] = self.db.session.query(
            Schedule_Meal_Model
        ).filter(Schedule_Meal_Model.meal_subscription_id == meal_subscription_id)
