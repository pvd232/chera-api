...
class Scheduled_Order_Meal_Repository(Base_Repository):
    def get_upcoming_scheduled_order_meals(
        ...
        scheduled_order_meals = (
            self.db.session.query(Scheduled_Order_Meal_Model)
            .filter(
                Scheduled_Order_Meal_Model.meal_subscription_id == meal_subscription_id,
                Scheduled_Order_Meal_Model.delivery_date
    ...
    def get_scheduled_order_meals(
        ...
        if meal_subscription_id:
            ...
            ] = self.db.session.query(Scheduled_Order_Meal_Model).filter(
                Scheduled_Order_Meal_Model.meal_subscription_id == meal_subscription_id
            )
            if scheduled_order_meals:
                return scheduled_order_meals
    ...
    def get_scheduled_order_meals_for_week(
        ...
        weekly_scheduled_order_meals = (
            self.db.session.query(Scheduled_Order_Meal_Model)
            .filter(
                Scheduled_Order_Meal_Model.meal_subscription_id == meal_subscription_id,
                Scheduled_Order_Meal_Model.delivery_date == delivery_date,
            )
            .all()
        )
    ...
    def create_scheduled_order_meals(
        self, scheduled_order_meal_domains: list["Scheduled_Order_Meal_Domain"]
    ) -> None:
    ...
    def update_home_page_scheduled_order_meals(
        ...
        updated_scheduled_order_meals: list["Scheduled_Order_Meal_Domain"],
    ) -> None:
        ...
        for current_scheduled_order_meal in current_scheduled_order_meals:
            current_scheduled_order_meals_map[str(current_scheduled_order_meal.id)] = ""
        ...
        for updated_scheduled_order_meal in updated_scheduled_order_meals:
            ...
            if (
                ...
                not in current_scheduled_order_meals_map
            ):
                new_scheduled_order_meal = Scheduled_Order_Meal_Model(
        ...
        for scheduled_order_meal_id in current_scheduled_order_meals_map.keys():
            scheduled_order_meal_to_delete = (
                self.db.session.query(Scheduled_Order_Meal_Model)
                .filter(Scheduled_Order_Meal_Model.id == UUID(scheduled_order_meal_id))
                .first()
            )
    ...
    def update_scheduled_order_meals(
        self, scheduled_order_meals: list["Scheduled_Order_Meal_Domain"]
    ) -> None:
        for scheduled_order_meal in scheduled_order_meals:
            scheduled_order_meal_to_update: Scheduled_Order_Meal_Model = (
                self.db.session.query(Scheduled_Order_Meal_Model)
                .filter(Scheduled_Order_Meal_Model.id == scheduled_order_meal.id)
                .first()
            )
            scheduled_order_meal_to_update.delivery_skipped = (
                scheduled_order_meal.delivery_skipped
            )
        self.db.session.commit()
    ...
    def pause_scheduled_order_meals(self, meal_subscription_id: UUID) -> None:
        ...
        for scheduled_order_meal in scheduled_order_meals_to_pause:
            if self.can_pause_scheduled_order_meal(
                scheduled_order_meal=scheduled_order_meal
            ):
    ...
    def delete_scheduled_order_meals(
        ...
        for scheduled_order_meal in scheduled_order_meals_to_delete:
            # Only future scheduled order meals can be deleted
            if scheduled_order_meal.delivery_date > today:
    ...
    def can_pause_scheduled_order_meal(
        self, scheduled_order_meal: "Scheduled_Order_Meal_Domain"
    ) -> bool:
        if datetime.utcfromtimestamp(scheduled_order_meal.delivery_date).replace(
            tzinfo=timezone.utc
