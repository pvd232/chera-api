...
class Dietitian_Prepayment_Service(object):
    def __init__(self, dietitian_prepayment_repository: 'Dietitian_Prepayment_Repository') -> None:
        self.dietitian_prepayment_repository = dietitian_prepayment_repository

    def set_prepayment_values(self, num_meals: int,  dietitian_prepayment_domain: Dietitian_Prepayment_Domain, meal_price: float, shipping_price: float, discount_percentage: float = None) -> Dietitian_Prepayment_Domain:
        service_fee = 0.0
        sales_tax_total = 0.0
        ...
        dietitian_prepayment_domain.datetime = datetime.now(timezone.utc)
        return dietitian_prepayment_domain

    def create_dietitian_prepayment(self, num_meals: int, discount_code: str, dietitian_id: str, staged_client_id: str, stripe_payment_intent_id: str, meal_price: float, shipping_price: float, discount_service: 'Discount_Service', prepaid_order_discount_service: 'Prepaid_Order_Discount_Service') -> None:
        ...
        dietitian_prepayment_dto = Dietitian_Prepayment_DTO(dietitian_prepayment_json={
            "id": uuid(),
        ...
        pre_update_new_dietitian_prepayment_domain.stripe_payment_intent_id = stripe_payment_intent_id
        post_update_new_dietitian_prepayment_domain = self.set_prepayment_values(
            num_meals=num_meals, dietitian_prepayment_domain=pre_update_new_dietitian_prepayment_domain, discount_percentage=discount_percentage, meal_price=meal_price, shipping_price=shipping_price)
        ...
        if discount:
            prepaid_order_discount_service.create_prepaid_order_discount(discount_id=discount.id, dietitian_prepayment_id=post_update_new_dietitian_prepayment_domain.id,
                                                                         discount_percentage=discount.discount_percentage, num_meals=num_meals, meal_price=meal_price)
        return
