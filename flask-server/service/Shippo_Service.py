...
class Shippo_Service(object):
    def create_shipment(self, meal_subscription_invoice_id: str, client: 'Client_Domain', meal_shipment_repository: 'Meal_Shipment_Repository') -> Optional['Meal_Shipment_Model']:
        ...
        parcel = shippo.Parcel.create(
            length=12, width=9, height=7, distance_unit="in", mass_unit="lb", weight="10")

        shipment = shippo.Shipment.create(
            address_from=address_from,
            address_to=address_to,
            parcels=parcel, asynchronous=False)
        ...
        transaction = shippo.Transaction.create(
            rate=rate.object_id,
            label_file_type="PDF", asynchronous=False)
        # Retrieve label url and tracking number
        if transaction.status == "SUCCESS":
            # create new shipment object with tracking number and label url
            meal_shipment_dict = {"id": uuid(), "meal_subscription_invoice_id": meal_subscription_invoice_id, "shippo_transaction_id": transaction.object_id,
                                  "label_url": transaction.label_url, "tracking_number": str(transaction.tracking_number), "tracking_url": transaction.tracking_url_provider}
            meal_shipment_domain = Meal_Shipment_Domain(
                meal_shipment_json=meal_shipment_dict)
            meal_shipment_repository.create_shipment(
                meal_shipment=meal_shipment_domain)
