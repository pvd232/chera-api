...
class Date_Service(object):
    def __init__(self) -> None:
        ...
        self.first_email_cutoff_difference = self.delivery_cutoff_hour_index - \
            self.first_email_hour_index
        ...
        if self.delivery_day_index > self.shipping_day_index:
            self.delivery_shipping_day_difference = self.delivery_day_index - self.shipping_day_index
        else:
            self.delivery_shipping_day_difference = (
                self.delivery_day_index + 7) - self.shipping_day_index
        self.delivery_shipping_hour_difference = self.delivery_hour_index - \
            self.shipping_hour_index
        ...
        if self.delivery_day_index > self.delivery_cutoff_day_index:
            self.delivery_cutoff_day_difference = self.delivery_day_index - \
                self.delivery_cutoff_day_index

        else:
            self.delivery_cutoff_day_difference = (
                self.delivery_day_index + 7) - self.delivery_cutoff_day_index
        self.delivery_cutoff_hour_difference = self.delivery_hour_index - \
            self.delivery_cutoff_hour_index
    ...
    def get_current_week_delivery_date(self) -> float:
        ...
        if days_until_delivery_day < 0 or (days_until_delivery_day == 0 and hours_until_delivery_day <= 0):
            days_until_delivery_day += 7
        current_week_delivery_date = datetime(
            year=today.year, month=today.month, day=today.day, hour=self.delivery_hour_index, tzinfo=timezone.utc) + timedelta(days=days_until_delivery_day)
    ...
    def get_current_week_cutoff(self, current_delivery_date: float) -> float:
        current_delivery_datetime = datetime.utcfromtimestamp(
            current_delivery_date).replace(tzinfo=timezone.utc)
        current_week_cutoff = datetime(
            year=current_delivery_datetime.year, month=current_delivery_datetime.month, day=current_delivery_datetime.day, hour=current_delivery_datetime.hour, tzinfo=timezone.utc) - timedelta(days=self.delivery_cutoff_day_difference, hours=self.delivery_cutoff_hour_difference)
        return current_week_cutoff.timestamp()

    def get_stripe_delivery_date_anchor(self) -> float:
        today = datetime.now(timezone.utc)
        current_week_anchor = datetime.utcfromtimestamp(self.get_current_week_cutoff(
            current_delivery_date=self.get_current_week_delivery_date())).replace(tzinfo=timezone.utc)
    ...
    def get_next_week_delivery_date(self, current_delivery_date: float) -> float:
        new_datetime = datetime.utcfromtimestamp(
            current_delivery_date).replace(tzinfo=timezone.utc) + timedelta(days=7)
        return new_datetime.timestamp()

    def get_shipping_date_from_delivery_date(self, current_delivery_date: float) -> float:
        shipping_date = datetime(
            year=current_delivery_date.year, month=current_delivery_date.month, day=current_delivery_date.day, hour=current_delivery_date.hour, tzinfo=timezone.utc) - timedelta(days=self.delivery_shipping_day_difference, hours=self.delivery_shipping_hour_difference)
    ...
    def get_upcoming_delivery_dates(self) -> list[float]:
        ...
        for i in range(0, 4):
            upcoming_delivery_date = datetime.utcfromtimestamp(
                current_delivery_date).replace(tzinfo=timezone.utc) + timedelta(days=i*7)
            upcoming_delivery_dates.append(
                upcoming_delivery_date.timestamp())
    ...
    def get_upcoming_cutoff_delivery_dates(self) -> list[float]:
        ...
        current_delivery_date = self.get_current_week_delivery_date()
        for i in range(0, 4):
            upcoming_cutoff_date = datetime.utcfromtimestamp(self.get_current_week_cutoff(
                current_delivery_date=current_delivery_date)).replace(tzinfo=timezone.utc) + timedelta(days=i*7)
            upcoming_cutoff_delivery_dates.append(
                upcoming_cutoff_date.timestamp())
    ...
    def get_first_email_datetime(self) -> float:
        ...
        first_email_datetime = datetime.utcfromtimestamp(current_week_cutoff).replace(
            tzinfo=timezone.utc) + timedelta(hours=self.first_email_cutoff_difference)
