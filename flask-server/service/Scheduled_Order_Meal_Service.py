...
class Scheduled_Order_Meal_Service(object):
    def __init__(self, scheduled_order_meal_repository: 'Scheduled_Order_Meal_Repository') -> None:
        self.scheduled_order_meal_repository = scheduled_order_meal_repository

    def update_scheduled_order_meals(self, scheduled_order_meal_dtos: list['Scheduled_Order_Meal_DTO']) -> None:
        scheduled_order_meal_domains = [Scheduled_Order_Meal_Domain(
            scheduled_order_meal_object=x, schedule_meal_object=None, scheduled_order_meal_id=None, delivery_date=None, is_paused=None) for x in scheduled_order_meal_dtos]
    ...
    def update_home_page_scheduled_order_meals(self, scheduled_order_meal_dtos: list['Scheduled_Order_Meal_DTO']) -> None:
        scheduled_order_meal_domains = [Scheduled_Order_Meal_Domain(
            scheduled_order_meal_object=x, schedule_meal_object=None, scheduled_order_meal_id=None, delivery_date=None, is_paused=None) for x in scheduled_order_meal_dtos]
        self.scheduled_order_meal_repository.update_home_page_scheduled_order_meals(meal_subscription_id=scheduled_order_meal_domains[0].meal_subscription_id,
    ...
    def delete_scheduled_order_meals(self, meal_subscription_id: UUID, cutoff_date: float, current_week_delivery_date: float, is_first_week: bool) -> None:
        self.scheduled_order_meal_repository.delete_scheduled_order_meals(
            meal_subscription_id=meal_subscription_id, cutoff_date=cutoff_date, current_week_delivery_date=current_week_delivery_date, is_first_week=is_first_week)
        return
    ...
    def create_scheduled_order_meals(self, scheduled_order_meal_dtos: list['Scheduled_Order_Meal_DTO']) -> None:
        scheduled_order_meal_domains = [Scheduled_Order_Meal_Domain(
            scheduled_order_meal_object=x, schedule_meal_object=None, scheduled_order_meal_id=None, delivery_date=None, is_paused=None) for x in scheduled_order_meal_dtos]
        self.scheduled_order_meal_repository.create_scheduled_order_meals(
    ...
    def get_upcoming_scheduled_order_meals(self, meal_subscription_id: UUID) -> Optional[list[Scheduled_Order_Meal_Domain]]:
        scheduled_order_meals: Optional[list['Scheduled_Order_Meal_Model']] = self.scheduled_order_meal_repository.get_scheduled_order_meals(
            meal_subscription_id=meal_subscription_id)
        if scheduled_order_meals:
            return [Scheduled_Order_Meal_Domain(scheduled_order_meal_object=x, schedule_meal_object=None, scheduled_order_meal_id=None, delivery_date=None, is_paused=None) for x in scheduled_order_meals]
        else:
            return None

    def get_scheduled_order_meals(self, meal_subscription_id: Optional[UUID]) -> Optional[list[Scheduled_Order_Meal_Domain]]:
        scheduled_order_meals: Optional[list['Scheduled_Order_Meal_Model']] = self.scheduled_order_meal_repository.get_scheduled_order_meals(
            meal_subscription_id=meal_subscription_id)
        if scheduled_order_meals:
            return [Scheduled_Order_Meal_Domain(scheduled_order_meal_object=x, schedule_meal_object=None, scheduled_order_meal_id=None, delivery_date=None, is_paused=None) for x in scheduled_order_meals]
    ...
    def get_scheduled_order_meals_for_week(self, meal_subscription_id: UUID, delivery_date: float) -> Optional[list[Scheduled_Order_Meal_Domain]]:
        weekly_scheduled_order_meal_models = self.scheduled_order_meal_repository.get_scheduled_order_meals_for_week(
            meal_subscription_id=meal_subscription_id, delivery_date=delivery_date)
        if weekly_scheduled_order_meal_models:
            weekly_scheduled_order_meal_domains: list[Scheduled_Order_Meal_Domain] = [
                Scheduled_Order_Meal_Domain(scheduled_order_meal_object=x, schedule_meal_object=None, scheduled_order_meal_id=None, delivery_date=None, is_paused=None) for x in weekly_scheduled_order_meal_models]
            return weekly_scheduled_order_meal_domains
        else:
            return None

    def get_current_scheduled_order_meal_delivery_dates(self, meal_subscription_id: UUID) -> Optional[list[float]]:
        scheduled_order_meals: Optional[list[Scheduled_Order_Meal_Domain]] = self.get_scheduled_order_meals(
            meal_subscription_id=meal_subscription_id)
        if scheduled_order_meals:
            date_dict = {}
            for meal in scheduled_order_meals:
                # Check if delivery date is in the future and not already in the dictionary
                if meal.delivery_date > datetime.now(timezone.utc).timestamp() and meal.delivery_date not in date_dict:
    ...
    def refresh_scheduled_order_meals(self, meal_subscription_id: UUID, schedule_meal_service: 'Schedule_Meal_Service', date_service: 'Date_Service', is_paused: bool) -> None:
        ...
        if len(delivery_dates) == 4:
            ...
            for schedule_meal in schedule_meals:
                ...
                new_scheduled_order_meal = Scheduled_Order_Meal_Domain(scheduled_order_meal_object=None,
                                                                       schedule_meal_object=schedule_meal, scheduled_order_meal_id=new_scheduled_order_meal_id, delivery_date=date_service.get_next_week_delivery_date(last_scheduled_order_meal_date), is_paused=is_paused)
