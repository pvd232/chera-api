...
class Extended_Meal_Plan_Meal_Service(Meal_Plan_Meal_Service):
    def get_extended_meal_plan_meal(self, meal_plan_meal_id: UUID) -> Extended_Meal_Plan_Meal_Domain:
        return Extended_Meal_Plan_Meal_Domain(meal_plan_meal_model=self.meal_plan_meal_repository.get_meal_plan_meal(meal_plan_meal_id=meal_plan_meal_id))

    def get_extended_meal_plan_meals(self) -> Optional[list['Extended_Meal_Plan_Meal_Domain']]:
        meal_plan_meals: Optional[list['Meal_Plan_Meal_Model']
                                  ] = self.meal_plan_meal_repository.get_meal_plan_meals()
        if meal_plan_meals:
    ...
    def compute_new_meal_plan_meal(self, meal_plan_meal_id: UUID, updated_recipe: list['Extended_Recipe_Ingredient_DTO']) -> Extended_Meal_Plan_Meal_Domain:
        ...
        for recipe_ingredient in updated_recipe:
            updated_recipe_dict[recipe_ingredient.usda_ingredient_id] = recipe_ingredient
        ...
        for recipe_ingredient in unaltered_extended_meal_plan_meal_domain.recipe:
            ...
            if matching_ingredient.quantity != recipe_ingredient.quantity:
                difference = matching_ingredient.quantity / \
