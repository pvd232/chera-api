...
class Meal_Shipment_Domain(Base_Domain):
    def __init__(
        self,
        meal_shipment_object: Optional[Meal_Shipment_Model] = None,
        meal_shipment_json: Optional[dict] = None,
    ) -> None:
        if meal_shipment_object:
            self.id = meal_shipment_object.id
            self.meal_subscription_invoice_id = (
                meal_shipment_object.meal_subscription_invoice_id
            )
            ...
            self.meal_subscription_invoice_id = meal_shipment_json[
