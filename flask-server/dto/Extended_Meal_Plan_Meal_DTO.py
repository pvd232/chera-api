...
class Extended_Meal_Plan_Meal_DTO(Meal_Plan_Meal_DTO):
    def __init__(self, extended_meal_plan_meal_domain: 'Extended_Meal_Plan_Meal_Domain') -> None:
        super().__init__(meal_plan_meal_domain=extended_meal_plan_meal_domain)
        self.recipe: list[Extended_Recipe_Ingredient_DTO] = [Extended_Recipe_Ingredient_DTO(
            extended_recipe_ingredient_domain=x)for x in extended_meal_plan_meal_domain.recipe]
    ...
    def get_nutritional_k_cal_adjustment(self) -> None:
        ...
        nutritional_k_cal = self.get_fat_k_cal() + self.get_carb_k_cal() + \
            self.get_protein_k_cal()
    ...
    def serialize(self) -> dict:
        ...
        serialized_attributes['protein_k_cal'] = round(self.get_adjusted_protein_k_cal(
        ))
        serialized_attributes['fat_k_cal'] = round(
            self.get_adjusted_fat_k_cal())
        serialized_attributes['carb_k_cal'] = round(
            self.get_adjusted_carb_k_cal())
