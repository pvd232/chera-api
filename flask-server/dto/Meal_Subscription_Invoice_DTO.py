...
class Meal_Subscription_Invoice_DTO(Base_DTO):
    def __init__(self, meal_subscription_invoice_json: dict = None, meal_subscription_invoice_domain: 'Meal_Subscription_Invoice_Domain' = None) -> None:
        if meal_subscription_invoice_json:
            ...
            self.subtotal: float = float(
                meal_subscription_invoice_json["subtotal"])
            self.sales_tax_percentage: float = float(meal_subscription_invoice_json[
            ...
            self.datetime: float = float(
                meal_subscription_invoice_json["datetime"])
            ...
            self.meal_subscription_id: UUID = meal_subscription_invoice_domain.meal_subscription_id
            self.subtotal: float = meal_subscription_invoice_domain.subtotal
            self.sales_tax_percentage: float = meal_subscription_invoice_domain.sales_tax_percentage
            self.sales_tax_total: float = meal_subscription_invoice_domain.sales_tax_total
            self.shipping_total: float = meal_subscription_invoice_domain.shipping_total
            self.stripe_fee_total: float = meal_subscription_invoice_domain.stripe_fee_total
            self.stripe_invoice_id: str = meal_subscription_invoice_domain.stripe_invoice_id
            self.stripe_payment_intent_id: str = meal_subscription_invoice_domain.stripe_payment_intent_id
